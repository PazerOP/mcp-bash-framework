...
auto_first_total="$(jq -r 'select(.id=="auto-page-1") | .result.total' "${AUTO_ROOT}/responses-1.ndjson")"
...
jq -s '
	...
	if ($list.total != 2) then err("expected two templates after collision/override filtering") else null end,
