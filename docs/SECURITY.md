...
- HTTPS provider hardening: private/loopback hosts are blocked; optional allow/deny lists via `MCPBASH_HTTPS_ALLOW_HOSTS` / `MCPBASH_HTTPS_DENY_HOSTS`; timeouts and size are bounded (timeouts capped at 60s, max bytes capped at 20MB), redirects/protocol downgrades disabled. Hostnames are resolved before fetch; obfuscated private IPs (e.g., decimal) are rejected.
