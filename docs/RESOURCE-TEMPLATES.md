...
- `resources/templates/list` supports `limit` (default 50, max 200) and includes `total` alongside `resourceTemplates` and `nextCursor` (cursor uses the templates registry hash; stale cursors return `-32602`).
