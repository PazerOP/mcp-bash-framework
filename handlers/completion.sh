...
mcp_handle_completion() {
  ...
  case "${method}" in
    completion/complete)
      ...
      start_offset=0
      if [ -n "${cursor}" ]; then
        start_offset="$(env CURSOR_VALUE="${cursor}" "${py}" <<'PY'
...
PY
      ...
      prepared="$(env NAME="${name}" ARGS_JSON="${args_json}" LIMIT="${limit}" START="${start_offset}" "${py}" <<'PY'
import json, os
...
PY
      ...
      if [ "${has_more_flag}" = "true" ]; then
        ...
        if [ -n "${next_index}" ]; then
          mcp_completion_cursor="$(env NAME="${name}" NEXT="${next_index}" ARGS_JSON="${args_json}" "${py}" <<'PY'
import base64, json, os
payload = json.dumps({
    "name": os.environ.get("NAME"),
      ...
      while IFS= read -r suggestion; do
        ...
        if ! mcp_completion_add_json "${suggestion}"; then
          ...
          if [ -z "${mcp_completion_cursor}" ]; then
            ...
            mcp_completion_cursor="$(env NAME="${name}" NEXT="${next_offset}" ARGS_JSON="${args_json}" "${py}" <<'PY'
import base64, json, os
payload = json.dumps({
    "name": os.environ.get("NAME"),
    "next": int(os.environ.get("NEXT", "0")),
...
PY
        ...
        added=$((added + 1))
      done < <(env PAYLOAD="${limited_json}" "${py}" <<'PY'
import json, os
